...
Thankfully, that's not the case. Draqula combines all errors into one object and sets error's `message` property to equal the message of the first error returned from server.
...
Keep in mind though, that `GraphQLError` is a "bundle" error that contains multiple errors inside.
So if you expect multiple errors per query, you need to convert `error` returned from `useQuery` into an array and iterate it to display all of them.
