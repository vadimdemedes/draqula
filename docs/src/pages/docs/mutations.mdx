...
const Blog = () => {
	const [posts] = useQuery(POSTS_QUERY);
	const [pages] = useQuery(PAGES_QUERY);
...
`useMutation` hook returns an array of 4 items: `[mutate, data, loading, error]`
...
Type: `NetworkError | GraphQLError`<br/>
Default: `null`
