...
This hook allows you to modify request before it's sent to the server.
...
This hook lets you check or modify response before Draqula starts processing it.
You could use `afterResponse` hooks to log requests, report errors to Sentry (or other error tracking service) or reauthenticate user with a fresh access token.
...
Check out more examples in [ky's repository](https://github.com/sindresorhus/ky#hooksafterresponse).
