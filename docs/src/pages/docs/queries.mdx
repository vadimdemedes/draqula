...
Queries in Draqula are executed via `useQuery` React hook, exposed from the `draqula` module:

```jsx
import {useQuery} from 'draqula';
```

All queries **must** be using `gql` tag function from the [`graphql-tag`](https://github.com/apollographql/graphql-tag) module and they **must** be defined outside your component.
...
- **data** - Query response
- **isLoading** - Boolean that indicates if query is still loading
...
So in cases when there's cache for a query, `isLoading` will equal to `false`, even when query is refetching in the background.
...
To prevent you from managing the loading state of `fetchMore` manually (since `isLoading` will remain `false` when `fetchMore` is called), Draqula exposes `isFetchingMore` variable.
It's the same as `isLoading`, but only reflects the state of `fetchMore` operation.
