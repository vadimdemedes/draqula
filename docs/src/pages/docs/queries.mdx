...
You've probably noticed `useQuery` doesn't return `loading` or a similar field.
The reason for that is simple - you don't need one.
If `data` is `undefined`, query is loading, otherwise query was either loaded or failed.
...
const Todos = () => {
	...
	const {data, error} = useQuery(TODOS_QUERY, {page});
...
```jsx
const Todos = () => {
	const {data, error, refetch} = useQuery(TODOS_QUERY);
...
Note that refetching a query doesn't reset its current `data` value.
It's implemented this way, so that user still sees data on the screen, even if new data is coming.
...
const Todos = () => {
	const {data, error, fetchMore, isFetchingMore} = useQuery(TODOS_QUERY, {
...
To prevent you from managing the loading state of `fetchMore` manually (since `data` will remain unchanged when `fetchMore` is called), Draqula exposes a `isFetchingMore` variable.
It only reflects the state of the `fetchMore` operation, not the fetching of initial data when component mounts.
...
const Todos = () => {
	const {data, error, fetchMore} = useQuery(TODOS_QUERY);
...
Useful for keeping data on your page up-to-date when user comes back.
...
If `undefined` is returned, query is loading.
